...
function mtaStsAuxiliaryResponse(request: Request, url: URL): Response | null {
  ...
  if (url.pathname === mtaStsPolicyPath) {
    return new Response(request.method === "HEAD" ? null : mtaStsPolicy, { status: 200, headers });
  }
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const mtaStsAuxiliary = mtaStsAuxiliaryResponse(request, url);
    if (mtaStsAuxiliary) {
      return withSecurityHeaders(mtaStsAuxiliary, request);
    }
